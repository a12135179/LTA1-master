...
class Config:
    ...
    def _file2dict(filename):
        ...
        if filename.endswith('.py'):
            with tempfile.TemporaryDirectory(dir='C:/Users/User/Desktop/lane/LTA-master') as temp_config_dir:
                print('temp_config_dir=',temp_config_dir)
                temp_config_file = tempfile.NamedTemporaryFile(
                    dir=temp_config_dir, suffix='')
                temp_config_name = osp.basename(temp_config_file.name)
                shutil.copyfile(filename,osp.join(temp_config_dir, temp_config_name)+'.py')
